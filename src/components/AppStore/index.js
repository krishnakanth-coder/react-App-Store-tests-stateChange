...
class AppStore extends Component {
  state = {isActive: tabsList[0].tabId, searchInput: ''}

  onclickSelectApps = id => {
    const activeAppsButton = tabsList.filter(
      eachTabList => eachTabList.tabId === id,
    )
    this.setState({isActive: activeAppsButton[0].tabId})
  }
  ...
  render() {
    ...
    const userSearchApps = this.filterApps()
    return (
          ...
          <ul className="list-card">
            {tabsList.map(eachTab => (
              <TabItem
                eachTab={eachTab}
                key={eachTab.tabId}
                onclickSelectApps={this.onclickSelectApps}
                isActive={isActive}
              />
            ))}
          </ul>
          <ul className="app-card-container">
            {userSearchApps.map(eachApp => (
